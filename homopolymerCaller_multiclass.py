...
with tf.name_scope('input'):
    ...
    y_placeholder = tf.placeholder(tf.int64, [batch_size, nb_steps])
...
with tf.name_scope('training'):
    losses = [tf.nn.sparse_softmax_cross_entropy_with_logits(logits=pr, labels=yi) for pr, yi in zip(y_hat, y)]
...
with tf.name_scope('Performance_assessment'):
    ...
    y_hat_stacked = tf.argmax(tf.stack(y_hat, axis=-1), axis=1)
    ...
    for i in range(num_classes):
        cm = tf.constant(i, shape=[batch_size, nb_steps], dtype=tf.int64)
        y_bin = tf.cast(tf.equal(y_stacked, cm), dtype=tf.int64)
        y_hat_bin = tf.cast(tf.equal(y_hat_stacked, cm), dtype=tf.int64)
...
with tf.Session() as sess:
    ...
    while epoch_index <= num_epochs:
        ...
        while i+input_size < raw.size:
            ...
            # cur_onehot[onehot[i+label_shift]] = 1
        ...
        if step % 10 == 0:
            ...
            if step % 100 == 0:
                ...
                ts_plot = helper_functions.plot_timeseries(raw, num_classes, label_shift, y_hat, batch_size, nb_steps, base_labels)
